---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the baldwin project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the baldwin project.
...
- `` :py:mod:`baldwin.string` `` for modules
- `` :py:func:`baldwin.string.slugify` `` for functions
- `` :py:class:`baldwin.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~baldwin.string.slugify` `` renders as `slugify`
