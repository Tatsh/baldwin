...
If any changed files are under `baldwin/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `baldwin/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `baldwin/commands/main.py`,
...
For Python files, strip the `baldwin/` prefix and replace `/` with `.` (like module imports).

- Python file `baldwin/media.py` → `media:`.
- Multiple files under `baldwin/commands/` → `commands:`.
- Single command file `baldwin/commands/admin.py` → `commands.admin:`.
